...
/// Batch delete multiple contents from storage
///
/// # Arguments
///
/// * `node` - The Codex node to use
/// * `cids` - List of content IDs to delete
///
/// # Returns
///
/// A vector of results, one for each CID
pub async fn batch_delete(node: &CodexNode, cids: &[String]) -> Vec<Result<()>> {
    let mut results = Vec::with_capacity(cids.len());

    for cid in cids {
        let result = delete(node, cid).await;
        results.push(result);
    }
...
/// Batch check if multiple contents exist in storage
///
/// # Arguments
///
/// * `node` - The Codex node to use
/// * `cids` - List of content IDs to check
///
/// # Returns
///
/// A vector of results, one for each CID
pub async fn batch_exists(node: &CodexNode, cids: &[String]) -> Vec<Result<bool>> {
    let mut results = Vec::with_capacity(cids.len());

    for cid in cids {
        let result = exists(node, cid).await;
        results.push(result);
    }
...
/// Validate a CID format for storage operations
///
/// # Arguments
///
...
/// # Returns
///
/// Ok(()) if the CID is valid, or an error
pub fn validate_cid(cid: &str) -> Result<()> {
    ...
    // CIDs typically start with known prefixes and are base58 encoded
    let valid_prefixes = vec![
        "Qm",     // CIDv0
        "bafy",   // CIDv1 raw
        "bafk",   // CIDv1 dag-pb
        "bafybe", // CIDv1 raw (longer)
        "bafkre", // CIDv1 raw (even longer)
        "zdj",    // CIDv1 libp2p-key
    ];

    let has_valid_prefix = valid_prefixes.iter().any(|&prefix| cid.starts_with(prefix));

    if !has_valid_prefix {
        return Err(CodexError::invalid_parameter(
            "cid",
            "CID has invalid format or prefix",
        ));
    }

    // Check for valid base58 characters (for CIDv0) or base32 (for CIDv1)
    if cid.starts_with("Qm") {
            ...
            .all(|c| "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".contains(c))
        {
            return Err(CodexError::invalid_parameter(
                "cid",
                "CIDv0 contains invalid base58 characters",
            ));
        }
    } else if cid.starts_with("baf") || cid.starts_with("zdj") {
        // CIDv1 uses base32
        if !cid
            .chars()
            .all(|c| "abcdefghijklmnopqrstuvwxyz234567".contains(c))
        {
            return Err(CodexError::invalid_parameter(
                "cid",
                "CIDv1 contains invalid base32 characters",
            ));
        }
    }
...
mod tests {
    ...
    async fn test_batch_operations() {
        ...
        // Test batch exists
        let exists_results = batch_exists(&node, &test_cids).await;
        assert_eq!(exists_results.len(), 3);
        for result in exists_results {
            assert!(result.is_ok());
            assert!(!result.unwrap()); // None should exist in a new node
        }

        // Test batch delete
        let delete_results = batch_delete(&node, &test_cids).await;
        assert_eq!(delete_results.len(), 3);
        // Results might be ok or err depending on implementation
    ...
    fn test_validate_cid() {
        ...
        for cid in valid_cids {
            assert!(validate_cid(cid).is_ok(), "CID {} should be valid", cid);
        }
        ...
        let invalid_cids = vec![
            "",
            "invalid",
            "QmInvalid@Characters",
            "bafyInvalid@Characters",
            "zdjInvalid@Characters",
            "Qm", // Too short
        ];

        for cid in invalid_cids {
            assert!(validate_cid(cid).is_err(), "CID {} should be invalid", cid);
        }
    }
}
